...
def search_document_links(query: str, scheme_name: str) -> list[dict]:
    """
    Search Bright Data SERP API for document/application links for a scheme.
    Returns a list of dicts with title, url, and snippet.
    Falls back to empty list silently on any failure.
    """
    if settings.offline_only:
        return []
    ...
    try:
        search_query = f"{scheme_name} apply online documents official site"
